...
const Downloads: React.FC<DownloadsProps> = ({ data, darkMode, variants }) => {
  ...
  return (
    ...
    <div className={`w-full min-h-screen pb-12 md:pb-20 transition-colors duration-500 ${darkMode ? 'bg-slate-950' : 'bg-slate-50'}`}>
      ...
      <div className="relative w-full min-h-[45vh] md:h-[50vh] overflow-hidden flex flex-col">
        ...
        <div className="absolute inset-0 bg-slate-900">
           ...
           <div className={`absolute inset-0 bg-linear-to-b ${darkMode ? 'from-transparent via-slate-950/60 to-slate-950' : 'from-transparent via-slate-50/30 to-slate-50'}`} />
        </div>
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-4 pt-12 pb-16">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-white tracking-tighter drop-shadow-xl mb-3 md:mb-4 leading-tight">
              TEDABLIUK <span className="text-teal-400 drop-shadow-[0_0_15px_rgba(45,212,191,0.5)]">WORLD</span>
            </h1>
            
            <p className="text-base md:text-xl text-slate-100 font-medium leading-relaxed drop-shadow-md px-2 max-w-lg mx-auto">
              Explore construções épicas, farms automáticas e segredos escondidos.
            </p>
          </motion.div>
        </div>
        ...
        >
          ...
          <div className="mb-10 md:mb-16">
             <div className="flex items-center gap-3 mb-6 md:mb-8">
                <div className={`p-2.5 md:p-3 rounded-2xl shadow-inner ${darkMode ? 'bg-teal-500/10 text-teal-400' : 'bg-teal-50 text-teal-600'}`}>
                  <DownloadIcon className="w-5 h-5 md:w-6 md:h-6" />
                </div>
                <div>
                   <h2 className={`text-xl md:text-2xl font-bold ${darkMode ? 'text-white' : 'text-slate-800'}`}>Escolha sua Versão</h2>
                   <p className={`text-xs md:text-sm ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Clique para expandir e baixar</p>
                </div>
             </div>
          ...
          <hr className={`border-0 h-px mb-10 md:mb-16 ${darkMode ? 'bg-slate-800' : 'bg-slate-100'}`} />

          {/* Seção 2: Guia de Teleporte */}
          <div>
            <div className="text-center mb-8 md:mb-10">
               <span className={`text-xs font-bold uppercase tracking-widest ${darkMode ? 'text-yellow-400' : 'text-yellow-600'}`}>
                 Exploração
               </span>
               <h2 className={`text-2xl md:text-4xl font-black mt-1 md:mt-2 ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                 Guia Turístico
               </h2>
